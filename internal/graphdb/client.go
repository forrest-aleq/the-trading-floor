	...
	return c.executeWrite(ctx, func(tx neo4j.ManagedTransaction) error {
		if err := runQuery(ctx, tx, `
			...
			SET b.origin_desk = $origin_desk,
			    ...
			    b.relationship_health = $relationship_health,
			    b.recovery_score = $recovery_score,
			    b.positive_recoveries = $positive_recoveries,
			    b.negative_violations = $negative_violations,
			...
			map[string]any{
				...
				"trust":               state.Trust,
				"confidence":          state.Confidence,
				"relationship_health": state.RelationshipHealth,
				"recovery_score":      state.RecoveryScore,
				"positive_recoveries": state.PositiveRecoveries,
				"negative_violations": state.NegativeViolations,
				"success_count":       state.SuccessCount,
				"failure_count":       state.FailureCount,
				"updated_at":          updatedAt,
			},
	...
	result, err := session.ExecuteRead(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
		rows, err := tx.Run(ctx, `
			...
			RETURN b.key AS key,
			       ...
			       coalesce(b.relationship_health, 0.5) AS relationship_health,
			       coalesce(b.recovery_score, 0.0) AS recovery_score,
			       coalesce(b.positive_recoveries, 0) AS positive_recoveries,
			       coalesce(b.negative_violations, 0) AS negative_violations,
		...
		for rows.Next(ctx) {
			...
			beliefs = append(beliefs, &model.DeskRelationshipBelief{
				...
				RelationshipHealth: toFloat(recordValue(record, "relationship_health")),
				RecoveryScore:      toFloat(recordValue(record, "recovery_score")),
				PositiveRecoveries: toInt(recordValue(record, "positive_recoveries")),
				NegativeViolations: toInt(recordValue(record, "negative_violations")),
...
func (c *Client) linkThesisMarketContext(ctx context.Context, tx neo4j.ManagedTransaction, thesis *model.Thesis, now time.Time) error {
	...
	if err := runQuery(ctx, tx, `
			...
			SET m.symbol = $symbol,
			    ...
			    m.ask_price = $ask_price,
			    m.mid_price = $mid_price,
			    m.spread_bps = $spread_bps,
			    m.last_volume = $last_volume,
			    m.quote_age_seconds = $quote_age_seconds,
			    m.return_15m_pct = $return_15m_pct,
			    m.return_1h_pct = $return_1h_pct,
			    m.return_4h_pct = $return_4h_pct,
			    m.realized_vol_1d_pct = $realized_vol_1d_pct,
			    m.realized_vol_5d_pct = $realized_vol_5d_pct,
		...
		map[string]any{
			...
			"bid_price":              marketCtx.BidPrice,
			"ask_price":              marketCtx.AskPrice,
			"mid_price":              marketCtx.MidPrice,
			"spread_bps":             marketCtx.SpreadBps,
			"last_volume":            marketCtx.LastVolume,
			"quote_age_seconds":      marketCtx.QuoteAgeSeconds,
			"return_15m_pct":         marketCtx.Return15mPct,
			"return_1h_pct":          marketCtx.Return1hPct,
			"return_4h_pct":          marketCtx.Return4hPct,
			"realized_vol_1d_pct":    marketCtx.RealizedVol1dPct,
			"realized_vol_5d_pct":    marketCtx.RealizedVol5dPct,
