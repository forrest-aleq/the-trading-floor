...
RUNTIME_PACKS_DIR="$RUNTIME_HOME/deploy/packs/trading-v1"

mkdir -p "$LAUNCH_AGENTS_DIR" "$RUNTIME_HOME/bin" "$RUNTIME_HOME/var/log" "$RUNTIME_HOME/var/audit" "$RUNTIME_MIGRATIONS_DIR" "$RUNTIME_PACKS_DIR"
...
if [[ "${LAUNCHD_INSTALL_ONLY:-}" != "1" ]]; then
...
echo "stderr: $LOG_ERR"
if [[ "${LAUNCHD_INSTALL_ONLY:-}" == "1" ]]; then
  echo "launchd install-only mode: service files updated but not bootstrapped"
fi
