...
func (ib *IB) PlaceOrder(contract *Contract, order *Order) *Trade {
	...
	// Subscribe and register the trade before sending. TWS can return
	// OrderStatus/Error callbacks immediately, and the previous send-before-store
	// ordering could drop those callbacks and leave the Trade in ApiPending.
	ch, unsubscribe := ib.pubSub.Subscribe(order.OrderID)
	go func(orderID int64, trade *Trade, ch <-chan string, unsubscribe UnsubscribeFunc) {
		defer unsubscribe()
		for {
			select {
			case <-trade.Done():
	...
	}(order.OrderID, trade, ch, unsubscribe)
